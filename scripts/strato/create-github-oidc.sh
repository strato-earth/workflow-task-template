...
tofu apply -input=false -auto-approve  -var="region=$REGION" -var="profile=$PROFILE" -var="environment=$ENVIRONMENT" -var="github_organization=$GITHUB_ORGANIZATION" -var="repo_name=$REPO_NAME" -var="oidc_provider_arn=$OIDC_PROVIDER_ARN" -var="build_artifacts_bucket=$ARTIFACTS_BUCKET"
